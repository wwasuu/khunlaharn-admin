...
class EditEvent extends React.Component {
  ...
  _getEvent = async () => {
    try {
      const res = await EventAPI.getById(this.props.router.query.id)
      this.setState({
        event: {
          ...res.data.event,
        ...
        images: res.data.event.media ? res.data.event.media : [],
        featuredImage: (res.data.event.featured_image_id && res.data.event.featured_image) ? {
  ...
  _handleSubmit = (e) => {
    ...
    this.props.form.validateFields((err, data) => {
      ...
      this._storeEvent(data)
    });
  }

  _storeEvent = async (data) => {
    try {
      const payload = {
        ...data,
        status: this.state.event.status ? 1 : 0,
        highlight: this.state.event.highlight ? 1 : 0,
      ...
      const res = await EventAPI.edit(payload)
      Router.push({
        pathname: '/events',
      })
    } catch (e) {
  ...
  _changeStatus = () => {
    this.setState({
      event: {...this.state.event, status: !this.state.event.status}
    })
  }

  _changeHighlight = () => {
    this.setState({
      event: {...this.state.event, highlight: !this.state.event.highlight}
    })
  }
  ...
  render() {
    ...
    const { isShowGallery, isLoading, event, galleryType, images, featuredImage } = this.state
    if (!event) {
      return null
    }
    return (
            ...
            <div className="form">
              ...
              <div className="_pdl-8px">
                ...
                {/* <div className="ant-row ant-form-item ant-form-item-no-colon">
                  ...
                  <Switch checked={event.highlight} onChange={this._changeHighlight} />
                </div> */}
                <div className="ant-row ant-form-item ant-form-item-no-colon">
                  <div className="ant-form-item-label">
...
export default Form.create()(withRouter(EditEvent))